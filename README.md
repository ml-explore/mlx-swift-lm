...
DeepSeek-OCR Swift code is adapted from **[mzbac/deepseek-ocr.swift](https://github.com/mzbac/deepseek-ocr.swift)** (MIT), not a clean-room rewrite from Python alone. mzbac copyright / license notices are retained in adapted file headers where substantial code is kept, and Python [mlx-vlm](https://github.com/Blaizzy/mlx-vlm) is the behavioral oracle used for parity.
...
Smoke: `MODE=base swift run DeepseekOCRSmoke` (or `MODE=gundam`, the default).
