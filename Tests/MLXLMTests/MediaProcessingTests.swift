...
public class MediaProcesingTests : XCTestCase
{
    ...
    {
        guard let fileURL = Bundle.module.url(forResource: "audio_only", withExtension: "mov") else {
            XCTFail("Missing file: 1080p_30.mov")
            return
        }
    ...
    func testVideoFileAsProcessedSequence() async throws
    {
        ...
        let frames = try await MediaProcessing.asProcessedSequence(video, samplesPerSecond: 2) { frame in
    ...
    func testVideoFramesAsProcessedSequence() async throws
    {
        ...
        func imageWithColor(_ color:CIColor) -> CIImage {
            ...
            return inputFilter.outputImage!.cropped(to: CGRect(x: 0, y: 0, width: 1920, height: 1080))
        }
        
        let colors:[CIColor] = [.red, .green, .blue, .cyan, .magenta, .yellow, .white, .black, .gray, .clear]
