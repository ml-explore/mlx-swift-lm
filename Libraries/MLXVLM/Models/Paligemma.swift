    ...
    private enum Vision {
        ...
        fileprivate class SigLipVisionModel: Module {
            ...
            public func callAsFunction(_ x: MLXArray, outputHiddenStates: Bool = false) -> (
                ...
                let (encoderOutput, hiddenStates) = encoder(
                    x, outputHiddenStates: outputHiddenStates)
    ...
    public struct PaliGemmaProcessor: UserInputProcessor {
        ...
        public func prepare(input: UserInput) throws -> LMInput {
            ...
            prompt =
                Array(repeating: "<image>", count: count).joined() + (tokenizer.bosToken ?? "")
                + prompt
                + "\n"
            ...
            let pixels = try prepare(
                image: input.images[0].asCIImage(), processing: input.processing)

            return LMInput(
                text: .init(tokens: promptArray, mask: mask), image: .init(pixels: pixels))
        }
