...
internal enum PixtralVision {
    ...
    class RotaryEmbedding {
        ...
        init(_ config: PixtralVisionConfiguration) {
            ...
            let tiledH = MLX.tiled(freqsH[0..., .newAxis, 0...], repetitions: [1, maxPatchesPerSide, 1])
            let tiledW = MLX.tiled(freqsW[.newAxis, 0..., 0...], repetitions: [maxPatchesPerSide, 1, 1])
    ...
    class EncoderLayer: Module {
        ...
        init(_ config: PixtralVisionConfiguration) {
            ...
            self._attentionNorm.wrappedValue = RMSNorm(dimensions: config.hiddenSize, eps: config.rmsNormEps)
    ...
    class PixtralVisionModelInner: Module {
        ...
        func callAsFunction(_ x: MLXArray, outputHiddenStates: Bool = false) -> (MLXArray, [MLXArray]?) {
...
public class PixtralVLM: Module, VLMModel, KVCacheDimensionProvider {
    ...
    @ModuleInfo(key: "multi_modal_projector") private var multiModalProjector: PixtralMultiModalProjector
