...
private enum Language {
    ...
    /// Helper to extract string from StringOrNumber
    static func stringValue(from value: StringOrNumber?) -> String? {
    ...
    /// YarnRoPE wrapper for Mistral3
    fileprivate class Mistral3YarnRoPE: Module {
        let dimensions: Int
        ...
        init(
            ...
            func yarnFindCorrectionDim(numRotations: Float) -> Float {
                Float(dimensions) * log(Float(originalMaxPositionEmbeddings) / (numRotations * 2 * Float.pi)) / (2 * log(base))
            }
            ...
            self._mscale = yarnGetMscale(scale: scalingFactor, mscale: mscale) / yarnGetMscale(scale: scalingFactor, mscale: mscaleAllDim)
            
            let freqExtra = pow(base, MLXArray(stride(from: 0, to: dimensions, by: 2)).asType(.float32) / Float(dimensions))
            let freqInter = scalingFactor * pow(base, MLXArray(stride(from: 0, to: dimensions, by: 2)).asType(.float32) / Float(dimensions))
        ...
        func callAsFunction(_ x: MLXArray, offset: Int = 0) -> MLXArray {
            ...
            return MLXFast.RoPE(x, dimensions: dimensions, traditional: traditional, base: nil, scale: 1.0, offset: offset, freqs: _freqs)
        }
    }
    ...
    fileprivate class Attention: Module {
        ...
        // Use optional properties for both RoPE types
        let rope: RoPE?
        let yarnRope: Mistral3YarnRoPE?
        ...
        init(_ config: Mistral3VLMTextConfiguration) {
            ...
            // Check for yarn RoPE using helper function
            let ropeType = Language.stringValue(from: config.ropeParameters?["rope_type"]) 
                ?? Language.stringValue(from: config.ropeParameters?["type"]) 
                ?? "default"
            let ropeTheta = config.ropeParameters?["rope_theta"]?.asFloat() ?? config.ropeTheta
            
            if ropeType == "yarn" {
                let factor = config.ropeParameters?["factor"]?.asFloat() ?? 1.0
                let origMax = config.ropeParameters?["original_max_position_embeddings"]?.asInt() ?? config.maxPositionEmbeddings ?? 4096
                let betaFast = config.ropeParameters?["beta_fast"]?.asFloat() ?? 32.0
                let betaSlow = config.ropeParameters?["beta_slow"]?.asFloat() ?? 1.0
                let mscale = config.ropeParameters?["mscale"]?.asFloat() ?? 1.0
                let mscaleAllDim = config.ropeParameters?["mscale_all_dim"]?.asFloat() ?? 0.0
                
                self.yarnRope = Mistral3YarnRoPE(
                    dimensions: headDim,
                ...
                // Default / linear RoPE scaling based on rope_parameters (matches initialize_rope)
                let ropeScale: Float = {
                    if ropeType == "linear",
                       let factor = config.ropeParameters?["factor"]?.asFloat(),
                       factor != 0 {
                        return 1.0 / factor
                    }
                    return 1.0
                ...
                self.rope = RoPE(
                    ...
                    traditional: config.ropeTraditional,
                    base: ropeTheta,
                    scale: (ropeScale == 1.0) ? 1.0 : ropeScale
                )
                self.yarnRope = nil
            }
        }
...
public class Mistral3VLM: Module, VLMModel, KVCacheDimensionProvider {
    ...
    private func getInputEmbeddings(
        ...
        print("[Mistral3 MODEL] getInputEmbeddings called")
        print("[Mistral3 MODEL] inputIds shape: \(inputIds?.shape ?? [])")
        print("[Mistral3 MODEL] pixelValues shape: \(pixelValues?.shape ?? [])")
        print("[Mistral3 MODEL] imageSizes: \(imageSizes ?? [])")
        ...
        guard var pixelValues, let imageSizes else {
            ...
            print("[Mistral3 MODEL] No image, returning text embeddings only")
        ...
        print("[Mistral3 MODEL] inputsEmbeds shape: \(inputsEmbeds.shape)")
        ...
        print("[Mistral3 MODEL] Vision tower returned \(hiddenStates.count) hidden states")
        let layerIndex = visionFeatureLayer < 0 
            ? hiddenStates.count + visionFeatureLayer 
            : visionFeatureLayer
        let selectedFeatures = hiddenStates[layerIndex]
        print("[Mistral3 MODEL] Selected layer \(layerIndex), features shape: \(selectedFeatures.shape)")
        ...
        print("[Mistral3 MODEL] After projector, imageFeatures shape: \(imageFeatures.shape)")
    ...
    private func mergeInputIdsWithImageFeatures(
        ...
        print("[Mistral3 MODEL] mergeInputIdsWithImageFeatures called")
        print("[Mistral3 MODEL] imageTokenIndex: \(imageTokenIndex)")
        print("[Mistral3 MODEL] imageFeatures shape: \(imageFeatures.shape)")
        print("[Mistral3 MODEL] inputsEmbeds shape: \(inputsEmbeds.shape)")
        print("[Mistral3 MODEL] inputIds shape: \(inputIds.shape)")
        ...
        print("[Mistral3 MODEL] numImagePatches: \(numImagePatches)")
        print("[Mistral3 MODEL] Found \(imagePositions.count) image tokens at positions: \(imagePositions.prefix(10))...")
        ...
        guard imagePositions.count == numImagePatches else {
            fatalError("Image token count (\(imagePositions.count)) does not match image patches (\(numImagePatches)). Ensure the processor adds exactly numImagePatches image tokens.")
        }
...
public class Mistral3VLMProcessor: UserInputProcessor {
    ...
    private func preprocessImage(
        ...
        if newWidth != paddedWidth || newHeight != paddedHeight {
            let background = CIImage(color: .black).cropped(to: CGRect(x: 0, y: 0, width: paddedWidth, height: paddedHeight))
            let tx = 0.0
    ...
    public func prepare(input: UserInput) async throws -> LMInput {
        ...
        // DEBUG: Log image token info
        print("[Mistral3 DEBUG] Image token: '\(imageToken)' -> ID: \(imageTokenId)")
        print("[Mistral3 DEBUG] Messages: \(messages)")
        ...
        print("[Mistral3 DEBUG] After chat template (\(promptTokens.count) tokens)")
        print("[Mistral3 DEBUG] Decoded: \(decoded)")
        print("[Mistral3 DEBUG] First 20 tokens: \(Array(promptTokens.prefix(20)))")
        print("[Mistral3 DEBUG] Contains imageToken '\(imageToken)': \(decoded.contains(imageToken))")
        ...
        print("[Mistral3 DEBUG] Num image tokens: \(preprocessResult.numImageTokens)")
        ...
        if decoded.contains(imageToken) {
            print("[Mistral3 DEBUG] Found imageToken in decoded string, splitting and expanding")
            // Split by image token and re-encode with expanded image tokens
            let pieces = decoded.components(separatedBy: imageToken)
            print("[Mistral3 DEBUG] Split into \(pieces.count) pieces")
            ...
            for (index, piece) in pieces.enumerated() {
                ...
                if index < pieces.count - 1 {
                    expandedTokens.append(contentsOf: Array(repeating: imageTokenId, count: preprocessResult.numImageTokens))
                }
            }
            promptTokens = expandedTokens
        } else {
            print("[Mistral3 DEBUG] imageToken NOT found in decoded string, using fallback")
            ...
            if foundImageToken {
                print("[Mistral3 DEBUG] Found single imageTokenId in tokens, expanded")
                promptTokens = expandedTokens
            } else {
                print("[Mistral3 DEBUG] No image token found at all, inserting at start")
                ...
                promptTokens.insert(
                    contentsOf: Array(repeating: imageTokenId, count: preprocessResult.numImageTokens),
                    at: insertIndex
                )
            }
        }
        
        print("[Mistral3 DEBUG] Final token count: \(promptTokens.count)")
        print("[Mistral3 DEBUG] Image token positions: \(promptTokens.enumerated().filter { $0.element == imageTokenId }.map { $0.offset })")
        print("[Mistral3 DEBUG] Pixels shape: \(preprocessResult.pixels.shape)")
