...
public struct JambaConfiguration: Codable, Sendable {
    ...
    public init(from decoder: Decoder) throws {
        ...
        self.mambaConvBias = try container.decodeIfPresent(Bool.self, forKey: .mambaConvBias) ?? true
        self.layersBlockType = try container.decodeIfPresent([String].self, forKey: .layersBlockType)
...
private class JambaAttention: Module {
    ...
    public init(_ config: JambaConfiguration) {
        ...
        _qProj.wrappedValue = Linear(config.hiddenSize, config.numAttentionHeads * self.headDim, bias: false)
        _kProj.wrappedValue = Linear(config.hiddenSize, config.numKeyValueHeads * self.headDim, bias: false)
        _vProj.wrappedValue = Linear(config.hiddenSize, config.numKeyValueHeads * self.headDim, bias: false)
        _oProj.wrappedValue = Linear(config.numAttentionHeads * self.headDim, config.hiddenSize, bias: false)
    }
    
    public func callAsFunction(_ x: MLXArray, mask: MLXFast.ScaledDotProductAttentionMaskMode, cache: KVCache?) -> MLXArray {
...
private class JambaMambaMixer: Module {
    ...
    public init(_ config: JambaConfiguration) {
        ...
        _inProj.wrappedValue = Linear(self.hiddenSize, self.intermediateSize * 2, bias: self.useBias)
        ...
        let A = repeated(
            MLXArray(Array(1...self.ssmStateSize).map { Float($0) }).reshaped([1, self.ssmStateSize]),
    ...
    private func processSequence(_ x: MLXArray, convState: MLXArray?, ssmState: MLXArray?) -> (MLXArray, (MLXArray, MLXArray)) {
        let xz = inProj(x)
        ...
        if let convState = convState {
            ...
            xFull = padded(x, widths: [IntOrPair((0, 0)), IntOrPair((K - 1, 0)), IntOrPair((0, 0))])
        }
    ...
    public func callAsFunction(_ x: MLXArray, cache: MambaCache?) -> MLXArray {
        ...
        let (output, (newConvState, newSsmState)) = processSequence(x, convState: convState, ssmState: ssmState)
...
private class JambaDecoderLayer: Module {
    ...
    public init(_ config: JambaConfiguration, layerType: String) {
        ...
        _preFFLayerNorm.wrappedValue = RMSNorm(dimensions: config.hiddenSize, eps: config.rmsNormEps)
    }
...
private class JambaModelInner: Module {
    ...
    public init(_ config: JambaConfiguration) {
        ...
        _finalLayerNorm.wrappedValue = RMSNorm(dimensions: config.hiddenSize, eps: config.rmsNormEps)
            ...
            for (n, m) in mapping {
                for k in ["weight", "scales", "biases"] {
                    if sanitizedWeights["\(prefix).block_sparse_moe.experts.0.\(n).\(k)"] != nil {
                        ...
                        sanitizedWeights["\(prefix).block_sparse_moe.switch_mlp.\(m).\(k)"] = MLX.stacked(toJoin)
                    }
                }
            }
        }
