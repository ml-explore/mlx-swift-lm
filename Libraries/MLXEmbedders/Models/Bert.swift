...
public class BertModel: Module, EmbeddingModel {
    ...
    public func callAsFunction(
        ...
        let embedded = embedder(inp, positionIds: positionIds, tokenTypeIds: tokenTypeIds)

        // 2. Pass through Transformer Encoder
        let outputs = encoder(embedded, attentionMask: mask)

        // 3. Generate Output based on model mode
        if let lmHead {
            return EmbeddingModelOutput(hiddenStates: lmHead(outputs), pooledOutput: nil)
        } else {
            // "Pooling" takes the first token ([CLS]) hidden state and applies tanh(Linear(x))
            let clsToken = outputs[0..., 0]
            let pooled = tanh(pooler!(clsToken))
            return EmbeddingModelOutput(hiddenStates: outputs, pooledOutput: pooled)
        }
    }
