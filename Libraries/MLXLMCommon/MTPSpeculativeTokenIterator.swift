...
public struct MTPSpeculativeTokenIterator: TokenIteratorProtocol {
    ...
    mutating func speculateRound() {
        ...
        if verifyOnMainCache {
            ...
            // on a copied cache, then commit only the real prefix into the
            // canonical cache: bonus token plus the accepted draft tokens. The
            // finalToken is the next token to feed, so it is intentionally not
            // replayed here.
