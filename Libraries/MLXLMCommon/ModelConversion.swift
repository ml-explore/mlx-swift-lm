...
public struct ModelConversionQuantization: Sendable, Equatable {
    ...
    public init(bits: Int = 4, groupSize: Int = 64, mode: QuantizationMode = .affine) {
        self.bits = bits
...
public func convert(
    ...
    let layerQuantization = quantizeForModelConversion(model: model, options: options)
    eval(model)
...
package func updateModelConfigWithQuantization(
    ...
    layerQuantization: [String: ModelConversionQuantization]
) throws {
    ...
    var quantizationJSON: [String: JSONValue] = [
        ...
        "group_size": .int(quantization.groupSize),
        "mode": .string(quantizationModeName(quantization.mode)),
    ]
    for (path, value) in layerQuantization.sorted(by: { $0.key < $1.key }) {
        quantizationJSON[path] = .object([
            "bits": .int(value.bits),
            "group_size": .int(value.groupSize),
...
private func quantizeForModelConversion(
...
) -> [String: ModelConversionQuantization] {
    var layerQuantization = [String: ModelConversionQuantization]()
    ...
    quantize(
        ...
        filter: { path, module in
            ...
            guard isConvertibleQuantizationTarget(module, groupSize: quantization.groupSize)
            else {
                return nil
            }

            if quantization != defaultQuantization {
                layerQuantization[path] = quantization
            }
            return (quantization.groupSize, quantization.bits, quantization.mode)
        },
        apply: quantizeLayerForModelConversion)
