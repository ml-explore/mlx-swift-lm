...
private final class DiffusionGemmaDecoder: Module {
    ...
    var prefersLogitsSelfConditioning: Bool {
        embedTokens is QuantizedEmbedding
    }

    func selfConditioningWeight() -> MLXArray? {
        prefersLogitsSelfConditioning ? nil : embedTokens.weight
    }
    ...
    func selfConditioningEmbeddings(logits: MLXArray, weight: MLXArray) -> MLXArray {
        ...
        return matmul(probabilities, weight).asType(weight.dtype)
    ...
    KVCacheDimensionProvider
{
    ...
    public var diffusionPrefersLogitsSelfConditioning: Bool {
        model.decoder.prefersLogitsSelfConditioning
    }
    ...
    public func prepareDiffusion(_ input: LMInput, cache: [KVCache], windowSize: Int?) throws {
        ...
        let validIndices = try blockDiffusionPromptIndices(
            ...
            sequenceLength: tokens.dim(1),
            batchSize: tokens.dim(0),
            modelName: String(describing: Self.self))
        encodeIntoCache(
            validIndices.map { tokens.take($0, axis: 1) } ?? tokens,
            cache: cache,
    ...
    public func prepareDiffusion(
        ...
        let validIndices = try blockDiffusionPromptIndices(
            mask: attentionMask,
            sequenceLength: normalizedEmbeddings.dim(1),
            batchSize: normalizedEmbeddings.dim(0),
            modelName: String(describing: Self.self))
        let compactedEmbeddings =
            validIndices.map { normalizedEmbeddings.take($0, axis: 1) }
            ?? normalizedEmbeddings
        let compactedTokenTypes: MLXArray?
        if let validIndices, let multimodalTokenTypes {
            compactedTokenTypes = multimodalTokenTypes.take(
                validIndices, axis: multimodalTokenTypes.ndim - 1)
        } else {
            compactedTokenTypes = multimodalTokenTypes
        }
        ...
        if compactedTokenTypes == nil {
            ...
            if compactedEmbeddings.dim(1) > chunkSize {
                ...
                while start < compactedEmbeddings.dim(1) {
                    let end = Swift.min(start + chunkSize, compactedEmbeddings.dim(1))
                    encodeEmbeddingsIntoCache(
                        compactedEmbeddings[0..., start ..< end, 0...],
        ...
        encodeEmbeddingsIntoCache(
            compactedEmbeddings,
            multimodalTokenTypes: compactedTokenTypes,
            cache: cache)
    }
    ...
    public func newCache(parameters: GenerateParameters?) throws -> [KVCache] {
        try config.textConfig.layerTypes.map { layerType in
