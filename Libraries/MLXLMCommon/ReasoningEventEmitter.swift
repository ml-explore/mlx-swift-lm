...
public struct ReasoningEventEmitter: Sendable {
    ...
    /// True once an end delimiter has closed a *reasoning* span. Unlike
    /// ``isInsideReasoning``, this latches - so a caller (e.g. a think-then-call
    /// token collector) can detect a close even when an empty `<think></think>`
    /// resolves within a single ``process(_:)`` call, where sampling
    /// ``isInsideReasoning`` afterward reads `false` both before and after and the
    /// transient open is invisible.
    ...
    public mutating func process(_ chunk: String) -> [Segment] {
        ...
        scan: while true {
            ...
            case .outside, .inside:
                ...
                // Consume the marker, trim whitespace immediately after it, and
                // re-scan the remainder in the mode the marker put us in.
